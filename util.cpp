...
ADDR_PTR find_addr()
{
    ...
    for (int i = 0; i < 2048; i++) // iterate over all set tags until we can find the set we want. 200 sounds good.
    {
        intArray[i] = new int;
        ADDR_PTR testAddr = reinterpret_cast<ADDR_PTR>(intArray[i]);
        if (cache_set_index(testAddr) == 200)
        {
            std::cout << "Hit at address: " << testAddr << " with index: " << cache_set_index(testAddr) << std::endl;
            targetAddr = testAddr;
            breakpoint = i; // Store how many adresses it took so we can clear them all later.
            break;
        }
    }

    for (int i = 0; i < breakpoint - 1; i++) // clean the big array up.
    {
        delete intArray[i];
    }
